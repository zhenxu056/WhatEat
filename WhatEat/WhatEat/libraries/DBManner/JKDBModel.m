...
+ (BOOL)isExistInTable
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        if ([tableName containsString:@"."]) {
            NSRange rang = [tableName rangeOfString:@"."];
            tableName = [tableName substringWithRange:NSMakeRange(rang.location+1, tableName.length - rang.location-1)];
        }
...
#pragma mark - 数据库地址
    ...
    for (NSString *column in resultArray) {
        ...
        if ([name containsString:@"."]) {
            ...
            tableName = [name substringWithRange:NSMakeRange(rang.location+1, tableName.length - rang.location-1)];
        }
        NSString *sql = [NSString stringWithFormat:@"ALTER TABLE %@ ADD COLUMN %@ ",name,fieldSql];
        if (![db executeUpdate:sql]) {
            return NO;
        }
    }
...
- (BOOL)save
{
    ...
    [jkDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        self.id = res?[NSNumber numberWithLongLong:db.lastInsertRowId].intValue:0;
        NSLog(res?@"插入成功":@"插入失败");
    }];
    return res;
}
...
+ (BOOL)saveObjects:(NSArray *)array
{
    ...
    [jkDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        for (JKDBModel *model in array) {
            ...
            model.id = flag?[NSNumber numberWithLongLong:db.lastInsertRowId].intValue:0;
