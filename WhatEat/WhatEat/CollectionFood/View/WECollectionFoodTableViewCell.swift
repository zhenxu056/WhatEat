...
class WECollectionFoodTableViewCell: UITableViewCell {
    ...
    var model: WEFoodModel {
        ...
        set {
            ...
            timelabel.text = newValue.creatTime
            let data = NSData(base64Encoded: newValue.imageUrl, options: .ignoreUnknownCharacters)
            picImage.image = UIImage.init(data: data! as Data)
        }
    }
